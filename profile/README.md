# About

This organization holds most of the projects made during my ([@ShadowXPA](https://github.com/ShadowXPA)) academic career.  

## Repositories

Each repository holds all the code produced. Some may not work, others may require adicional setup. Either way, I will not troubleshoot, since it is here exclusively for archiving.  
The `README.md` files, inside each repository, contains the assignment, objective, team, final grade, and my conclusions about the work and what could be improved.  

## Coimbra Institute of Engineering (ISEC)

The [Coimbra Institute of Engineering](https://isec.pt) is a higher education polytechnic institution of engineering, based in Coimbra, Portugal [[1]](https://en.wikipedia.org/wiki/Instituto_Superior_de_Engenharia_de_Coimbra).  

Follow [this link](/isec/README.md) to view the courses I took during my stay in this school.
